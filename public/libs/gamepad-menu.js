...
export default {
    ...
    checkMenuInput: function() {
        ...
        gamepadProxy.getGamepads().forEach(gamepad => {
            if (gamepad.axes[1] > 0.5 || gamepad.buttons[13] && gamepad.buttons[13].value > 0) {
                if (!this.inAction) menuLib.moveDown();
                inActionNow = true;
            }
            if (gamepad.axes[1] < -0.5 || gamepad.buttons[12] && gamepad.buttons[12].value > 0) {
                if (!this.inAction) menuLib.moveUp();
                inActionNow = true;
            }
            if (gamepad.buttons[0] && gamepad.buttons[0].value > 0) {
                if (!this.inAction) menuLib.select();
                inActionNow = true;
            }
            if (gamepad.buttons[1] && gamepad.buttons[1].value > 0) {
